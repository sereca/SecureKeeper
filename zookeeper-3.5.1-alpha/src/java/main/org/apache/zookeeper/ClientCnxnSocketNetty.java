...
public class ClientCnxnSocketNetty extends ClientCnxnSocket {
    ...
    private class ZKClientPipelineFactory implements ChannelPipelineFactory {
        ...
        public ChannelPipeline getPipeline() throws Exception {
            ...
            //SECUREKEEPER: Add ZKClientCryptoHandler to the processing pipeline          
            if(ZKClientCryptoHandler.ACTIVE && System.getProperty("zpp","false").equalsIgnoreCase("true")){
            	LOG.info("Using ZPP");
            	pipeline.addLast("client encrypt", new ZKClientCryptoHandler());
            } else if (Boolean.getBoolean(ZooKeeper.SECURE_CLIENT)) {
                initSSL(pipeline);
            }
        ...
        private synchronized void initSSL(ChannelPipeline pipeline) throws SSLContextException {
            if (sslContext == null || sslEngine == null) {
                ...
                //SECUREKEEPER: Enable only ciphers comparable to the once used in the enclave
		sslEngine.setEnabledProtocols(new String[]{"TLSv1.2"});
		sslEngine.setEnabledCipherSuites(new String[]{"TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256", "TLS_RSA_WITH_AES_128_GCM_SHA256","TLS_ECDH_ECDSA_WITH_AES_128_GCM_SHA256", "TLS_ECDH_RSA_WITH_AES_128_GCM_SHA256",    "TLS_DHE_RSA_WITH_AES_128_GCM_SHA256","TLS_DHE_DSS_WITH_AES_128_GCM_SHA256"});
            }
