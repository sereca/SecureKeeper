        ...
        throws KeeperException, IOException, RequestProcessorException
    {
        ...
        switch (type) {
            ...
            case OpCode.createContainer: {
                ...
                String seqNumber = String.format(Locale.ENGLISH, "%010d", parentCVersion);

                if (createMode.isSequential()) {
                    //SECUREKEEPER: Jump into enclave to append sequence number
                    LOG.debug("Sequential node");
                    LOG.debug(System.getProperty("encryption", "nozpp"));
                    if(System.getProperty("encryption", "nozpp").equalsIgnoreCase("sgx")) {

                        // allocate +10Byte to save space for sequential number.
                        int length = path.length() + 10 + 1; 
                        ByteBuffer path_buffer = ByteBuffer.allocate(length + 8); // +8 Byte safety
                        path_buffer.put(path.getBytes());
                        path_buffer.put((byte)0x0);
                        path_buffer.put(seqNumber.getBytes());

                        LOG.info("Calling Sequential Enclave");

                        byte[] seqPath = SeqEnclave.getInstance().getSequentialPath2(path_buffer.array(), length);

                        path = new String(seqPath);
                        LOG.debug("Received sequential path: " + DatatypeConverter.printHexBinary(path.getBytes()));
                        path = path.trim();
                        LOG.debug("Received sequential path: " + DatatypeConverter.printHexBinary(path.getBytes()));
                    } else {
                        LOG.debug("sequential w/o enclave.");
                        path = path + seqNumber;
                    }
                }
