...
void deallocate_Buffer(struct buffer *b) {
	if (b->buff) {
#ifdef MMGT
		printf("%s:%s:%d: free(%p).\n", __FILE__, __FUNCTION__, __LINE__, b->buff);
#endif
		free(b->buff);
	}
	b->buff = 0;
...
static int resize_buffer(struct buff_struct *s, int newlen) {
	...
	//--- start of replace realloc
//	buffer = (char*) malloc(s->len);
//	memcpy(buffer, s->buffer, oldlen);
//	free(s->buffer);
	//--- end   of replace realloc

#ifdef MMGT
	printf("%s:%s:%d: %p = realloc(%p,%d).\n", __FILE__, __FUNCTION__, __LINE__, buffer, s->buffer, s->len);
#endif
...
int ia_deserialize_buffer(struct iarchive *ia, const char *name,
	...
	b->buff = malloc(b->len);
#ifdef MMGT
	printf("%s:%s:%d: %p = malloc(%d).\n", __FILE__, __FUNCTION__, __LINE__, b->buff, b->len);
#endif
	if (!b->buff) {
		return -ENOMEM;
	}
...
int ia_deserialize_string(struct iarchive *ia, const char *name, char **s) {
	...
	*s = malloc(len + 1);
#ifdef MMGT
	printf("%s:%s:%d: %p = malloc(%d).\n", __FILE__, __FUNCTION__, __LINE__, *s, len + 1);
#endif
	if (!*s) {
		return -ENOMEM;
	}
...
static struct iarchive ia_default = { ia_start_record, ia_end_record,
		ia_start_vector, ia_end_vector, ia_deserialize_bool, ia_deserialize_int,
		ia_deserialize_long, ia_deserialize_buffer, ia_deserialize_string };

static struct oarchive oa_default = { oa_start_record, oa_end_record,
		oa_start_vector, oa_end_vector, oa_serialize_bool, oa_serialize_int,
		oa_serialize_long, oa_serialize_buffer, oa_serialize_string };

struct iarchive *create_buffer_iarchive(char *buffer, int len) {
	struct iarchive *ia = malloc(sizeof(*ia));
#ifdef MMGT
	printf("%s:%s:%d: %p = malloc(%d).\n", __FILE__, __FUNCTION__, __LINE__, ia, sizeof(struct iarchive));
#endif
	struct buff_struct *buff = malloc(sizeof(struct buff_struct));
#ifdef MMGT
	printf("%s:%s:%d: %p = malloc(%d).\n", __FILE__, __FUNCTION__, __LINE__, buff, sizeof(struct buff_struct));
#endif
	...
	if (!buff) {
		free(ia);
#ifdef MMGT
		printf("%s:%s:%d: free(%p).\n", __FILE__, __FUNCTION__, __LINE__, ia);
#endif
		return 0;
	}
	*ia = ia_default;
...
struct oarchive *create_buffer_oarchive() {
...
#ifdef MMGT
	printf("%s:%s:%d: %p = malloc(%d).\n", __FILE__, __FUNCTION__, __LINE__, oa, sizeof(struct oarchive));
#endif
	struct buff_struct *buff = malloc(sizeof(struct buff_struct));
#ifdef MMGT
	printf("%s:%s:%d: %p = malloc(%d).\n", __FILE__, __FUNCTION__, __LINE__, buff, sizeof(struct buff_struct));
	...
	if (!buff) {
		free(oa);
#ifdef MMGT
		printf("%s:%s:%d: free(%p).\n", __FILE__, __FUNCTION__, __LINE__, oa);
#endif
		return 0;
	}
	*oa = oa_default;
...
#ifdef MMGT
	printf("%s:%s:%d: %p = malloc(%d).\n", __FILE__, __FUNCTION__, __LINE__, buff->buffer, 128);
#endif
	buff->len = 128;
	oa->priv = buff;
	return oa;
...
#ifdef MMGT
	printf("%s:%s:%d: free(%p).\n", __FILE__, __FUNCTION__, __LINE__, (*ia)->priv);
#endif
	free((*ia)->priv);
#ifdef MMGT
	printf("%s:%s:%d: free(%p).\n", __FILE__, __FUNCTION__, __LINE__, *ia);
...
void close_buffer_oarchive(struct oarchive **oa, int free_buffer) {
	if (free_buffer) {
		...
		if (buff->buffer) {
			free(buff->buffer);
#ifdef MMGT
			printf("%s:%s:%d: free(%p).\n", __FILE__, __FUNCTION__, __LINE__, buff->buffer);
#endif
		}
	}
	free((*oa)->priv);
#ifdef MMGT
	printf("%s:%s:%d: free(%p).\n", __FILE__, __FUNCTION__, __LINE__, (*oa)->priv);
#endif
	free(*oa);
#ifdef MMGT
	printf("%s:%s:%d: free(%p).\n", __FILE__, __FUNCTION__, __LINE__, *oa);
